    ...
    <aside className="w-64 glass-panel h-[calc(100vh-6rem)] hidden lg:flex flex-col p-4 animate-[float-slow_8s_ease-in-out_infinite]">
      ...
      <nav className="flex-1 space-y-2">
        {menuItems.map((item) => (
          <button
            key={item.id}
            onClick={() => onNavigate(item.id === 'admin' ? 'admin' : 'dashboard')}
            className={`w-full flex items-center gap-4 px-4 py-3 rounded-xl transition-all duration-300 ${
              (item.id === 'admin' ? currentPage === 'admin' : currentPage === 'dashboard' && item.id === 'dashboard')
          ...
          >
            <svg className={`w-5 h-5 ${(item.id === 'admin' ? currentPage === 'admin' : currentPage === 'dashboard' && item.id === 'dashboard') ? 'text-neon-blue' : 'text-gray-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
